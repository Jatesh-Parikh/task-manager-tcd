...
export const formatTime = (createdAt: string) => {
  ...
  else if (created.isSame(now.subtract(1, "days"), "day")) {
    return "Yesterday";
  } //  check if the item was created within the last 7 days
  else if (created.isAfter(moment().subtract(6, "days"))) {
    return created.fromNow();
  } // check if the item was created within the last 4 weeks
  else if (created.isAfter(moment().subtract(3, "weeks"), "week")) {
    return created.fromNow();
  } else {
